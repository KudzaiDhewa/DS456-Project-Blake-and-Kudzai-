...
```
=======
Because C is several decades older than Rust, the C libraries in our sample are generally more mature in terms of age and historical usage. To mitigate bias arising from differences in development lifecycle stage, we prioritized libraries that are foundational to their respective ecosystems. While the C libraries naturally have longer histories, the selected Rust libraries represent the standard, production-ready alternatives currently in wide usage.
>>>>>>> 55e45d811a0a6aa1e4dca15ed3e342057897a42c
...
*   **Accordion Logic:** For large code files, the algorithm preserves the "Head" (first 5 lines) and "Tail" (last 5 lines) of a change hunk while truncating the repetitive middle. This reduced token usage by over 60% while maintaining semantic context.

# Data Processing

## Commit Classification Using Large Language Models

To distinguish between different types of development activity, we classified each commit into one of five mutually exclusive categories: **Memory Safety & Robustness**, **Concurrency & Thread Safety**, **Logic & Correctness**, **Build/Refactor**, and **Feature & Value Add**.

This classification was performed using a large language model (LLM), which was prompted with the commit message and the summarized diff. In addition to categorical classification, the LLM assigned a relative **complexity score (1-5)** reflecting the cognitive effort required for the task.

## LLM Configuration

*   **Model Used:** Google Gemini 2.0 Flash Lie (Supervised-Fine-Tuning).
*   **Infrastructure:** Google Cloud Platform (Vertex AI).
*   **Training Strategy:** We fine-tuned the model on a "Gold Standard" dataset of ~240 manually labeled commits from `libxml2` and `quick-xml`.
*   **Synthetic Augmentation:** To address class imbalance (where "Concurrency" bugs are rare), we augmented the training set with high-quality synthetic examples of C and Rust concurrency bugs. This ensured the model could recognize these critical but infrequent patterns.
...
To assess classification reliability, the model's outputs were validated against a held-out test set. We utilized a **Human-in-the-Loop (HITL)** approach during the creation of the training data, where an expert reviewer (aided by ensemble LLM consensus) verified the labels.
